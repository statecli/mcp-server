...
function getMCPConfigPath() {
    const config = vscode.workspace.getConfiguration('statecli');
    const customPath = config.get('mcpConfigPath');
    if (customPath) {
        return customPath;
    }
...
async function setupMCPConfig(showMessage) {
    ...
    try {
        ...
        if (showMessage) {
            const action = await vscode.window.showInformationMessage('StateCLI MCP server configured! Reload window to activate.', 'Reload Window');
...
function showTools() {
    const panel = vscode.window.createWebviewPanel('statecliTools', 'StateCLI Tools', vscode.ViewColumn.One, {});
    panel.webview.html = `
        <!DOCTYPE html>
        <html>
        <head>
