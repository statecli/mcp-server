...
## Complete Tool Listing (31 Tools)
...
### `statecli_search_web`
[KNOWLEDGE] Search the web for documentation or answers. Captures queries to telemetry.
...
### `statecli_read_url`
[KNOWLEDGE] Fetch and read the text content of a URL. Captures visited URLs to telemetry.
...
### `statecli_join_session`
[SESSION] Join a shared multi-agent namespace so multiple agents can share checkpoints and history.
🔔 Trigger when: Starting a collaborative multi-agent task.
🔗 Chain with: statecli_leave_session() when done.

---

### `statecli_leave_session`
[SESSION] Leave the current shared session gracefully.
🔔 Trigger when: Completing a collaborative task.
🔗 Chain with: statecli_join_session() to join another.
